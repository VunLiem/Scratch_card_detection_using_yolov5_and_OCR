...
# -------------------------------------looping through the detections------------------------------------
    for i in range(n):
        ...
        if row[4] >= 0.55:  ### threshold value for detection. We are discarding everything below this value
            ...
            cv2.putText(frame, f"{scratch_card_num}", (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,225), 2)
...
def filter_text(region, ocr_result, region_threshold):
    ...
    # Saving text only into file ocr_results.csv
    save_results([ocr_result[0][-2], 'ocr_detection1.csv','Detection_Image'])
    ...
    print("Results of OCR detection inside the bounding box: " + ocr_result[0][-2] +'\n'+"Accurate detector value: " + str(ocr_result[0][-1]))
...
main(img_path="E:/train/8.jpg")  ## for image
